import { db } from "./db";
import {
  users, employees, vehicles, categories, catalogItems, events, eventItems, eventTeam,
  ...
  CreateCatalogItemRequest, UpdateCatalogItemRequest, CatalogItemResponse,
  CreateEventRequest, UpdateEventRequest, EventResponse,
  AddEventItemRequest, AddEventTeamRequest,
  UserResponse,
} from "@shared/schema";
...
export interface IStorage {
  ...
  updateEmployee(id: string, updates: UpdateEmployeeRequest): Promise<EmployeeResponse>;
  ...
  updateVehicle(id: string, updates: UpdateVehicleRequest): Promise<VehicleResponse>;
  ...
  updateCategory(id: string, updates: UpdateCategoryRequest): Promise<CategoryResponse>;
  deleteCategory(id: string): Promise<void>;

  getCatalogItems(): Promise<CatalogItemResponse[]>;
  getCatalogItem(id: string): Promise<CatalogItemResponse | undefined>;
  createCatalogItem(item: CreateCatalogItemRequest): Promise<CatalogItemResponse>;
  updateCatalogItem(id: string, updates: UpdateCatalogItemRequest): Promise<CatalogItemResponse>;
  ...
  addEventTeamMember(eventId: string, teamData: AddEventTeamRequest): Promise<void>;
...
export class DatabaseStorage implements IStorage {
  ...
  async updateEmployee(id: string, updates: UpdateEmployeeRequest): Promise<EmployeeResponse> {
  ...
  async updateVehicle(id: string, updates: UpdateVehicleRequest): Promise<VehicleResponse> {
  ...
  async updateCategory(id: string, updates: UpdateCategoryRequest): Promise<CategoryResponse> {
  ...
  async createCatalogItem(item: CreateCatalogItemRequest): Promise<CatalogItemResponse> {
    const [inserted] = await db.insert(catalogItems).values(item).returning();
    return inserted;
  }

  async updateCatalogItem(id: string, updates: UpdateCatalogItemRequest): Promise<CatalogItemResponse> {
  ...
  async updateEvent(id: string, updates: UpdateEventRequest): Promise<EventResponse> {
  ...
  async addEventItem(eventId: string, itemData: AddEventItemRequest): Promise<void> {
    const catalogItem = await db.query.catalogItems.findFirst({ where: eq(catalogItems.id, itemData.catalogItemId) });
  ...
  async removeEventItem(eventId: string, itemId: string): Promise<void> {
    await db.delete(eventItems).where(and(eq(eventItems.id, itemId), eq(eventItems.eventId, eventId)));
    await this.recalculateEventFinancials(eventId);
  }

  async addEventTeamMember(eventId: string, teamData: AddEventTeamRequest): Promise<void> {
    const employee = await db.query.employees.findFirst({ where: eq(employees.id, teamData.employeeId) });
  ...
  async removeEventTeamMember(eventId: string, teamId: string): Promise<void> {
    await db.delete(eventTeam).where(and(eq(eventTeam.id, teamId), eq(eventTeam.eventId, eventId)));
    await this.recalculateEventFinancials(eventId);
  }
  ...
  private async recalculateEventFinancials(eventId: string) {
    ...
    for (const item of event.items) {
      totalRevenue += parseFloat(item.unitPriceSnapshot as string) * item.quantity;
      totalCostItems += parseFloat(item.unitCostSnapshot as string) * item.quantity;
    }
    ...
    for (const member of event.team) {
      ...
      if (event.transportType === 'INDIVIDUAL_TRANSPORT') {
        totalCostTransport += parseFloat(member.transportCostSnapshot as string);
      }
    }
    ...
    if (event.transportType === 'FLEET_VEHICLE' && event.vehicle) {
      ...
      const maintenance = parseFloat(event.vehicle.maintenanceCostPerKm as string);
    ...
    const netProfit = totalRevenue - (totalCostItems + totalCostLabor + totalCostTransport + extraExpenses);
